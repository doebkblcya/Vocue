...
/**
 * 面试官这一轮说的话不一定是新问题：可能只是应了一声、寒暄一句、
 * 说了点跟面试无关的话，或者一句话只说了半截。
 *
 * 这里**不列词表**。列词表既列不全，也会把「你确定吗」这类短但真实的反问误伤。
 * 判断权交给模型——它手上有前几轮的问答，够它做这个判断。
 */
const ANSWER_SCOPE = `回答前先看这一轮面试官说了什么：
1. 是一道完整的新问题：直接回答它，不要掺进之前的题目。
2. 只是应声、寒暄、和面试无关的闲聊，或者一句话只说了半截：不要凭空凑一段，也不要只回应这几个字。往前看最近几轮，找到最近一个真正值得回答的问题，把它和这一轮合起来，给出一份完整、可以直接说出口的回答。
3. 无论哪种情况都要给出答案，不要解释你在做哪种判断。`
